...
async def send_verification_code(
    ...
    try:
        verification = await whatsapp_service.send_verification_code(
            phone_number=data.phone_number, user_id=current_user.id, db=db
        )
...
async def verify_code(
    ...
    is_valid = await whatsapp_service.verify_code(
        phone_number=data.phone_number, code=data.code, db=db
    )

    if not is_valid:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Código inválido ou expirado"
        )
...
async def resend_verification_code(
    ...
    try:
        ...
        return {"success": True, "message": "Novo código enviado via WhatsApp"}
