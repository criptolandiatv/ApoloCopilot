...
async def create_category(
    ...
    category = ForumCategory(name=name, description=description, slug=slug)
...
async def get_threads(
    category_id: Optional[int] = None, limit: int = Query(20, le=100), db: Session = Depends(get_db)
):
    """Get forum threads"""
...
async def get_thread_posts(
    thread_id: int, limit: int = Query(50, le=200), db: Session = Depends(get_db)
):
...
async def create_post(
    ...
    post = ForumPost(thread_id=thread_id, author_id=current_user.id, content=data.content)
